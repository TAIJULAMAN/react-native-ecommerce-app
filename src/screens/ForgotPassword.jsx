...
export default function ForgotPassword({ isDarkMode, onSubmit, onGoSignIn }) {
  return (
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.select({ ios: 'padding', android: undefined })}>
      <View style={[styles.container, { backgroundColor: isDarkMode ? '#0B0F14' : '#F6F8FB' }]}> 
        <Text style={[styles.title, { color: isDarkMode ? '#F0F7FF' : '#0B1A33' }]}>Forgot Password</Text>
        <Text style={[styles.subtitle, { color: isDarkMode ? '#9FB3C8' : '#6B7A90' }]}>Enter your email to receive an OTP</Text>
        ...
        <TouchableOpacity onPress={onGoSignIn} style={{ marginTop: 14, alignSelf: 'center' }}>
