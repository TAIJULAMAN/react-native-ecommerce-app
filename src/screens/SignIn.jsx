...
export default function SignIn({ isDarkMode, onSubmit, onGoSignUp, onGoForgot }) {
  return (
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.select({ ios: 'padding', android: undefined })}>
      <View style={[styles.container, { backgroundColor: isDarkMode ? '#0B0F14' : '#F6F8FB' }]}> 
        <Text style={[styles.title, { color: isDarkMode ? '#F0F7FF' : '#0B1A33' }]}>Sign In</Text>
        <View style={styles.group}>
          ...
          <TouchableOpacity onPress={onGoForgot}>
            <Text style={[styles.link, { color: isDarkMode ? '#9AC6FF' : '#2563EB' }]}>Forgot password?</Text>
          </TouchableOpacity>
        ...
        <View style={styles.footerRow}>
          <Text style={{ color: isDarkMode ? '#9FB3C8' : '#6B7A90' }}>Don't have an account?</Text>
          <TouchableOpacity onPress={onGoSignUp}>
            <Text style={[styles.link, { marginLeft: 6, color: isDarkMode ? '#9AC6FF' : '#2563EB' }]}>Sign Up</Text>
          </TouchableOpacity>
        </View>
      </View>
